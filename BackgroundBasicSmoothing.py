...
n = int(reltimes2d[-1]+j)  # n = the element in reltimes3d that gives the relative time in seconds that the map ends 
x = reltimes3d[:n]		# x = an array with the reltimes3d data, but only for the map time (cuts off part after map is done)
x1 = x[0::500]			# takes a slice of 1 data point for every 500 points
y = Bz3d_scld[:n]
y1 = y[0::500]
plt.plot(x, y, 'go', ms = 3)	# plots all data points for Bz (scaled), up until end of map time
plt.plot(x1, y1, 'ro', ms = 6)	# plots every 500th data point for Bz (scaled), up until end of map time
f = interpolate.interp1d(x1, y1, kind='cubic', fill_value='extrapolate')	# interpolates between the points given by the arrays x1 and y1

xnew = reltimes3d[:n]
xnew = xnew[0::50]
Bz3d_scld_int = f(xnew)	# passes a new set of x values to the interpolation function, outputs y (the interpolated data)
